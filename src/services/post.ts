...
export function getPosts() {
  const filePaths = getFilesWithExtension("./data/posts", ".mdx");
...
export function getPostsById(id: string) {
  const filePaths = getFilesByName("./data/posts", `${id}.mdx`);
...
export async function getPostContentById(id: string) {
  const postFilePath = getFilesByName("./data/posts", `${id}.mdx`);
